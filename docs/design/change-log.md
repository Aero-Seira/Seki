...
## 2026-08-27 - 玉米面粉前置链补全 v13（FLOUR-CHAIN-02）

- 补充 v13 遗漏：`culturaldelights:corn_dough` 玉米面团存在，且原配方直吃玉米棒（`c:crops/corn`×3+水），绕过了玉米粒与玉米面粉两层中间体。
- 新增 `seki:corn_flour` 玉米面粉（startup + 模型 + 材质 + 译名；材质沿用 KC 面粉蓝本整图色编为玉米黄）。
- 新增 `seki:grains/corn_kernels` 标签（三成员：文化美食玉米粒 / 玉米乐玉米粒 / 农与魅玉米粒），三台磨粉机（手摇石磨 1:1 / Create 动力磨粉 1:1 / F&C 绞肉机 STONE 4:1）据此出玉米面粉。
- 覆盖 `culturalrecipes:corn_dough`：3 玉米面粉 + 水桶 → 3 玉米面团（同产率）；玉米饼/玉米片/肉馅卷下游自动继承新链。
