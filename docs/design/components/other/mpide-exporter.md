...
- 分发状态：本地专用例外，不纳入 packwiz；公开仓库/可验证发行构件当前不可用
...
- **packwiz 约束**：该 JAR 只保留在维护者真实实例中，Git 通过 `*.jar` 忽略规则保护它；干净客户端安装不会获得此开发工具。
...
2. 当前公开 GitHub 来源不可访问，无法建立稳定且可审计的 packwiz 下载项；若未来公开发行，应补充固定版本 URL 与哈希后再纳入。
